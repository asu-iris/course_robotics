...
As shown in  {numref}`linki_kinematics1`, the kinetic energy  of Link $i$ is given by
...
Then the velocity of an elementary particle of link $i$ 
 can be expressed as
...
{eq}`equ.ke_total`, it leads to that the total kinetic energy $ \mathcal{T}_{\ell_{i}}$ of link $i$ will include the following three terms
...
```{admonition} Total kinetic energy of a robot arm
By summing the kinetic energies of all links, the total
kinetic energy of a robot arm is 

$$\mathcal{T}=\frac{1}{2} \dot{\boldsymbol{q}}^{T} \boldsymbol{B}(\boldsymbol{q}) \dot{\boldsymbol{q}}$$(equ.ke_total_final)
...
The potential energy of a robot arm is given by the sum of the potential energy of each
link:
...
```{admonition} Total potential energy of a robot arm
...
```{admonition} Vector Form
By applying the Lagrange formulation {eq}`equ.lf`,
...
If we look at the details of Lagrange formulation {eq}`equ.lf2`, and also notice  $\mathcal{U}$ does not depend on
$\dot{\boldsymbol{q}}$, then
...
```{admonition} Detailed Form
By applying the detailed form of Lagrange formulation {eq}`equ.lf2`, the dynamics equation is 
...
A physical interpretation to  {eq}`equ.arm_dyn2` 
...
There is another commonly-used form of dynamics equation derived from {eq}`equ.arm_dyn2`, which will be introduced below. The quadratic velocity term in  {eq}`equ.arm_dyn2` has
...
```{admonition} Christoffel-Symbols Form
